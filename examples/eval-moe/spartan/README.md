...
`--torch-backend=cu129` selects the PyTorch CUDA-12.9 wheel index
(which is the only one that publishes `torch==2.13.0`, the version
this vLLM branch pins in `pyproject.toml`'s `[build-system].requires`).
`-e .` installs the local `${REPO_ROOT}` checkout so any commits you
pushed to the GPFS source tree are picked up immediately. **A plain
